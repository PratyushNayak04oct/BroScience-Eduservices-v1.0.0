...
// right half. The institute logo and the footer rule straddle the gutter, so
// drawing them once is the only way they stay aligned across the two page meshes.
...
function stampLogoOnHalves(leftCanvas, rightCanvas, logoImage) {
  if (!logoImage) return;
  const size = 248;
  const y = 560;
  const half = size / 2;
  ...
  left.save();
  left.globalAlpha = 0.94;
  left.drawImage(
    logoImage,
    0, 0, logoImage.width / 2, logoImage.height,
    leftCanvas.width - half, y, half, size,
  );
  left.restore();

  const right = rightCanvas.getContext("2d");
  right.save();
  right.globalAlpha = 0.94;
  right.drawImage(
    logoImage,
    logoImage.width / 2, 0, logoImage.width / 2, logoImage.height,
    0, y, half, size,
  );
  right.restore();
}

function paintSpreadContent(ctx, copy, logoImage) {
...
export function createPaperCanvas() {
  ...
  edge.addColorStop(0,    "rgba(138,100,40,0.22)");
  edge.addColorStop(0.12, "rgba(138,100,40,0.03)");
  edge.addColorStop(0.88, "rgba(138,100,40,0.03)");
  edge.addColorStop(1,    "rgba(138,100,40,0.22)");
  ctx.fillStyle = edge;
  ctx.fillRect(0, 0, w, h);
...
export function createSpreadPair(copy = SPREAD_COPY, logoImage) {
  ...
  paintAgedSpread(ctx);
  paintSpreadContent(ctx, copy, logoImage);
  ...
  const left  = half(0);
  const right = half(halfW + FOLD_PX / 2);
  stampLogoOnHalves(left, right, logoImage);
  return { left, right };
}
